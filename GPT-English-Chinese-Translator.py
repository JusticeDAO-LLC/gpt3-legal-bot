...
DISCORD_TOKEN = "OTUzMjE2NjQ3MzQ5NTU5MzM2.YjBWLg.kDPnxKLB66fI5XDyOqdpZh5lci0"  # Get one here: https://discord.com/developers/applications/
...
ALLOWED_CHANNEL = 953216917479489547  # Yannic Kilcher "gpt3"
MESSAGE_CHANNEL = 953216917479489547  # Yannic Kilcher "bot-chat"
...
def detect_if_english_unicode(text: str) -> bool:
    return any(ord(char) > 128 for char in text)

def detect_if_english_text(text: str) -> bool:
    return any(char.isalpha() for char in text)

def call_gpt(prompt, settings):
    if detect_if_english_text(prompt) or detect_if_english_unicode(prompt):
        prompt = "Translate the following into Chinese: \n'''" + prompt + "'''\n"
